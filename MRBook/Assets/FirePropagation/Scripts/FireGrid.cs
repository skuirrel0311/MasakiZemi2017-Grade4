...
public class FireGrid : MonoBehaviour {
    ...
    public int m_widthCells = 45;
    [Tooltip("Number of cells in the gird (height).")]
    public int m_heightCells = 45;
    ...
    // call this after a new FireGrid script component is created (i.e. called from FireIgniter)
    ...
    // propagates the fire within this grid
    ...
    // propagates the fire within this grid without wind
    void propagation()
    {
        m_fireCellsLit = 0;
    ...
    // gets the fire propagation speed based on the terrain fuel type the cell is located on, also fuel, mositure and HP values
    ...
    // true if valid location to spawn fire otherwise false
    ...
    // returns the bias value when fire is affected by propagating into head on winds, clamped between 1.0f -> max value
    ...
    // returns the bias value when affected by a up hill slope
    float computeUpSlopeBias()
    {
        return m_hillBias + m_bias;
    }

    // returns the bias value when affected by a down hill slope
    float computeDownSlopeBias()
    {
        return m_hillBias - m_bias;
    }

    // is the fire propagating on a slope, if so which way (up or down) or is it possiable to propagate (i.e. slope to steep/cliff)
    // return:
    // 0 = flat
    // 1 = up hill
    // 2 = down hill
